    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .topTrailing) {
                ...
                ZStack {
                    // تم تصحيح الإحداثيات هنا لتعمل بدون أخطاء
                    Image("star")
                        .frame(width: 250, height: 250).scaleEffect(0.2).position(x: (geometry.size.width / 2) - 140, y: 260)
                    Image("star")
                        .frame(width: 250, height: 250).scaleEffect(0.2).position(x: (geometry.size.width / 2) - 70, y: 260)
                    Image("star")
                        .frame(width: 250, height: 250).scaleEffect(0.2).position(x: (geometry.size.width / 2), y: 260)
                    Image("star")
                        .frame(width: 250, height: 250).scaleEffect(0.2).position(x: (geometry.size.width / 2) + 70, y: 260)
                            ...
                            .onAppear {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                                    withAnimation(.easeInOut(duration: 1.5)) {
                                        starPosition = CGPoint(x: (geometry.size.width / 2) + 140, y: 260)
                                        starScale = 0.2
                                    }
                ...
                // المحتوى الأساسي (النص والصور)
                VStack(spacing: 0) {
                    Image("s").resizable().scaledToFit().frame(width: 500).padding(.top, 40)
                    
                    Spacer() // دفع المحتوى ليتوسط الشاشة
                    
                    VStack(spacing: 40) {
                        Text("ماذا تلبس المرأة في العمرة؟")
                            .font(.system(size: 35, weight: .bold)) // كبّرت الخط قليلاً
                        
                        HStack(spacing: 60) {
                            Button(action: { checkAnswer(isCorrect: false) }) {
                                Image("no1").resizable().scaledToFit().frame(width: 400, height: 400)
                                    .background(Color.white.opacity(0.3)).cornerRadius(25)
                            }
                            Button(action: { checkAnswer(isCorrect: true) }) {
                                Image("yes1").resizable().scaledToFit().frame(width: 400, height: 400)
                                    .background(Color.white.opacity(0.3)).cornerRadius(25)
                            }
                        }.disabled(isInteractionDisabled)
                    }
                    
                    Spacer() // رفع المحتوى عن الأمواج بالأسفل
                    Spacer()
