    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .topTrailing) {
                ...
                // ✅ 4) النجوم (رفعناها شوي عشان ما تدخل على السؤال)
                ZStack {
                    Image("star")
                        .frame(width: 250, height: 250)
                        .scaleEffect(0.2)
                        .position(x: (geometry.size.width / 2) - 105, y: 235)

                    Image("star")
                        .frame(width: 250, height: 250)
                        .scaleEffect(0.2)
                        .position(x: (geometry.size.width / 2) - 35, y: 235)
                            ...
                            .onAppear {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                                    withAnimation(.easeInOut(duration: 1.5)) {
                                        starPosition = CGPoint(x: (geometry.size.width / 2) + 105, y: 235)
                                        starScale = 0.2
                                    }
                ...
                // ✅ 5) المحتوى الرئيسي (مرتب زي الصفحات اللي قبل)
                VStack(spacing: 0) {
                    Image("s")
                        .resizable()
                        .scaledToFit()
                        .frame(width: 500)
                        .padding(.top, 100)   // ✅ نفس Quiz2/3 (يعطي مساحة)
