...
struct UmrahPathView: View {
    ...
    @State private var activeStep: StepID?
    ...
    var body: some View {
        GeometryReader { geo in
                    ...
                    ZStack {
                        UmrahCurvedPath()
                            .stroke(Color.gray.opacity(0.35),
                                    style: StrokeStyle(lineWidth: max(2, circleSize * 0.06),
                                                       lineCap: .round,
                                                       dash: [10, 10]))
                        ...
                        ForEach(1...5, id: \.self) { step in
                            StepNode(
                                ...
                                iconName: iconName(step: step),
                                onTap: { tapped in
                                    if tapped <= currentStep {
                                        activeStep = StepID(value: tapped)
                                    } else {
            ...
            .sheet(item: $activeStep) { item in
                StepDetailView(step: item.value) {
                    completeStep(item.value)
                }
            }
        }
    }
    ...
    private func iconName(step: Int) -> String {
        switch step {
        case 1: return "tshirt"
        case 2: return "building.columns"
        case 3: return "figure.walk"
        case 4: return "scissors"
        default: return "questionmark.bubble"
        }
    }
}
