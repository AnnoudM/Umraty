...
struct ThllPagegirl_suha: View {
  ...
  var body: some View {
    ...
    // 🔊 AUTO PLAY – SAME AS BOY TA HALLUL
    .onAppear {
      playGirlTahallulSound()
    }
  ...
  // 🎧 GIRL TAHALLUL AUDIO (AUTO)
  func playGirlTahallulSound() {
    guard let url = Bundle.main.url(forResource: "girl_tahallul", withExtension: "wav") else { return }
    player = try? AVAudioPlayer(contentsOf: url)
    player?.play()
  }
